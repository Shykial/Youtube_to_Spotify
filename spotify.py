...
class Spotify_API():
    ...
    def get_token(self) -> str:
        if input('Stworzyc nowy token? ')[0] not in 'ty':
...
''')
            ...
            with open('spotify_secrets.py', 'r') as f:
                ...
                for i, line in enumerate(contents):
                    if 's_token = ' in line:
                        ...
                        contents[i + 1] = f"s_token_expiry_date = '{token_exp_time.strftime(strftime_format)}'"
    ...
    # playlist_id = create_playlist(user_id, 'Nowa playlista', token)
    playlist_id = '45dFjOp0DcGTGTola7dOGI'
    print(playlist_id)
